...
它帮你把文章、观点、产品逻辑、品牌概念、故事脚本和数据结构，转成封面、配图、信息图、逻辑图、架构图、Logo、故事板和社交媒体图片。
...
它负责把内容变成更容易理解、更容易传播、更适合平台的视觉表达。
...
| 资产生成 | 调用图像模型、HTML/SVG 模板、Mermaid、p5.js 或本地脚本生成结果 |
...
- 凭据不入库：OpenAI、Google、DashScope、Gemini 等 API Key 只放在环境变量或本地配置中，不提交到仓库。
