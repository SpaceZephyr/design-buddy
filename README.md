...
- 一段故事应该切成哪些关键分镜
- 一个品牌应该生成哪些 Logo 方向，或参考哪套真实品牌设计规范
...
Design Buddy 内所有 Skill 都统一使用 `space-*` 命名，目录名与 `SKILL.md` 里的 `name` 保持一致。

- `space-`：表示这是 SpaceZephyr 体系下的设计类 Skill。
- 中间词：说明主要能力，例如 `chart`、`logo`、`wechat`、`brand`。
- 后缀词：说明输出或形态，例如 `html`、`image`、`deck`、`poster`、`diagram`。

例如：`space-chart-html` 生成 HTML 图表，`space-chart-image` 生成图片图表，`space-wechat-layout` 生成公众号排版页。
...
| `space-gemini-article-illustration` | Markdown 文章批量配图并插入文档 | 图片 + 更新后的文章 |
| `space-markdown-poster` | Markdown 转社交媒体分享海报 | PNG |
| `space-logo-generator` | 品牌 Logo 批量生成和方向探索 | Logo 图片 |
| `space-brand-ui-design` | 从 getdesign.md 获取 62 个顶级品牌的 DESIGN.md 规范，生成匹配品牌风格的 UI | HTML / React / Vue / CSS |
| `space-storyboard-generator` | 故事润色、分镜拆解、故事板生成 | 分镜表 + 图片 |
| `space-chart-html` | 流程图、架构图、ER 图、旅程图等 10 类图表 | HTML 图表 |
| `space-chart-html-plus` | 35+ 类图表，覆盖技术、流程、战略、数据和信息展示 | HTML 图表 |
| `space-chart-image` | 使用 GPT-image-2 生成流程图、架构图、SWOT、路线图等图片图表 | PNG |
| `space-architecture-diagram` | 中文系统架构图、部署图、网络拓扑图 | HTML / SVG |
| `space-text-logic-diagram` | 把文章或论述拆成递进、流程、层次、对比等逻辑图 | HTML / SVG |
...
| 视觉决策 | 选择图像类型、图表结构、平台比例、品牌规范、风格系统和输出媒介 |
...
- 品牌视觉：可以先抽取品牌属性批量探索 Logo，也可以通过 getdesign.md 拉取 Apple、Notion、Stripe、Linear 等品牌的真实 DESIGN.md 规范生成 UI。
...
- 独立开发者：做 Logo、落地页素材、品牌风格 UI、产品说明图
...
- 外部规范来源：`space-brand-ui-design` 会运行 `npx getdesign@latest` 获取公开品牌设计规范，需联网和 Node.js 环境；生成结果商用前仍要检查品牌授权边界。
