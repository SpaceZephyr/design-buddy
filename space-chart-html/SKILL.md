...
- 用户说"XX 风格"（品牌名） → 调用 space-brand-ui-design skill 获取设计 token，应用到图表中
...
1. 从 space-brand-ui-design skill 的注册表中找到 slug
...
- 保存路径：`00 收件箱/画板/space-chart-html/MMDD-主题.html`
  - 检查路径是否存在，不存在则创建
  - 主题名从图表标题中提取，2-4 个字
