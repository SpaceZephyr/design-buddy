---
name: space-image-skill-builder
...
| 用户选择 | 生成的 Skill 结构 |
|---------|-----------------|
| 文章配图 | 类似 space-gemini-article-illustration: 分析文章 → 拆段落 → 生成提示词 → 确认 → 批量出图 → 插入文档 |
| Logo 设计 | 类似 space-logo-generator: 收集品牌信息 → 选风格 → 10个变体 → 出图 → 迭代 |
| 分镜故事 | 类似 space-storyboard-generator: 润色故事 → 拆分镜 → 选风格 → 批量出图 |
| 社交媒体图 | 新模式: 输入主题/文案 → 选模板 → 批量出图 → 导出特定尺寸 |
| 海报/封面 | 新模式: 输入主题 → 选风格 → 生成多个变体 → 迭代 |
| 数据可视化 | 类似 space-gemini-article-illustration 但专注图表: 输入数据/概念 → 选图表类型 → 出图 |
